...
type ViewType = 'overview' | 'episodes' | 'drift' | 'export';
...
export function App() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const views: ViewType[] = ['overview','episodes','drift','export'];
      if (e.key>='1'&&e.key<='4') setSelectedView(views[parseInt(e.key)-1]);
      if (e.key==='r') loadData();
    };
  ...
  const metricsOverTime = useMemo(() => episodes.sort((a,b)=>a.timestamp-b.timestamp).slice(-30).map(ep=>({
    time:new Date(ep.timestamp).toLocaleTimeString(), duration:Math.round(ep.actualDuration), deadline:Math.round(ep.deadline)
  })), [episodes]);
  ...
  const driftDist = useMemo(() => drifts.reduce((acc,d) => {
    const ex=acc.find(x=>x.name===d.type); if(ex) ex.value++; else acc.push({name:d.type,value:1}); return acc;
  }, [] as Array<{name:string;value:number}>), [drifts]);

  const radarData = useMemo(() => metrics.map(m=>({
    agent:m.agentName.replace('Agent',''), success:m.successRate, freshness:m.averageFreshness, latency:100-(m.avgLatency/5)
  })), [metrics]);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <nav className="border-b border-slate-800 bg-slate-900">
        <div className="max-w-7xl mx-auto px-4 flex gap-1">
          {(['overview','episodes','drift','export'] as const).map((view,i) => (
            <button key={view} onClick={()=>setSelectedView(view)}
              ...
              <span className="text-xs text-slate-600 mr-1">{i+1}</span>{view.charAt(0).toUpperCase()+view.slice(1)}
            </button>
          ))}
        </div>
      </nav>
      ...
      <div className="fixed bottom-4 right-4 text-xs text-slate-600 bg-slate-900/80 px-3 py-1.5 rounded border border-slate-800">
        Press 1-4 to switch views \u00B7 R to refresh
      </div>
    </div>
  );
          }
