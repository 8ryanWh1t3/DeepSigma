    ...
    return (
          <div className="flex flex-col items-center gap-2">
                <svg width="100" height="100" viewBox="0 0 100 100">
                        ...
                        <text x="50" y="46" textAnchor="middle" fill={color} fontSize="18" fontWeight="700">{value.toFixed(0)}</text>text>
                        <text x="50" y="62" textAnchor="middle" fill="#94a3b8" fontSize="10">{label}</text>text>
                </svg>svg>
          </div>div>
        );
}
...
function KPICard({ icon, label, value, trend, trendUp }: KPICardProps) {
    return (
          <div className="bg-slate-900 rounded-lg border border-slate-800 p-4 hover:border-slate-700 transition-colors">
                ...
                <div className={`text-xs ${trendUp === true ? 'text-green-400' : trendUp === false ? 'text-red-400' : 'text-slate-500'}`}>{trend}</div>div>
          </div>div>
        );
}
    ...
    return (
          <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                        <h3 className="text-lg font-semibold">Recent Decision Episodes</h3>h3>
                        <div className="flex items-center gap-3">
                                  ...
                                  <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)}
                                              ...
                                              <option value="all">All Status</option>option><option value="success">Success</option>option>
                                              <option value="timeout">Timeout</option>option><option value="degraded">Degraded</option>option><option value="failed">Failed</option>option>
                                  </select>select>
                        </div>div>
                </div>div>
                <div className="text-xs text-slate-500 mb-2">Showing {filtered.length} episodes</div>div>
                <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          ...
                          <tr key={ep.episodeId} className="border-b border-slate-800 hover:bg-slate-800 transition-colors">
                                          ...
                                          <td className="py-2 px-3"><span className={`px-2 py-1 rounded text-xs font-medium ${ep.status==='success'?'bg-green-900/50 text-green-300':ep.status==='timeout'?'bg-red-900/50 text-red-300':ep.status==='degraded'?'bg-yellow-900/50 text-yellow-300':'bg-purple-900/50 text-purple-300'}`}>{ep.status}</span>span></td>td>
                                          ...
                                          <td className="py-2 px-3">
                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                <span className="text-xs">{ep.freshness.toFixed(1)}%</span>span>
                                                            </div>div>
                                          </td>td>
          ...
          <div className="space-y-4">
                <div className="grid grid-cols-3 gap-4">
                        <div className="bg-red-900/20 border border-red-800 rounded-lg p-4 text-center">
                                  <div className="text-2xl font-bold text-red-400">{counts.high}</div>div>
                                  <div className="text-xs text-red-300">High Severity</div>div>
                        </div>div>
                        <div className="bg-yellow-900/20 border border-yellow-800 rounded-lg p-4 text-center">
                                  <div className="text-2xl font-bold text-yellow-400">{counts.medium}</div>div>
                                  <div className="text-xs text-yellow-300">Medium Severity</div>div>
                        </div>div>
                        <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-4 text-center">
                                  <div className="text-2xl font-bold text-blue-400">{counts.low}</div>div>
                                  <div className="text-xs text-blue-300">Low Severity</div>div>
                        </div>div>
                </div>div>
                <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                        <div className="flex items-center justify-between mb-4">
                                  <h3 className="text-lg font-semibold flex items-center gap-2"><AlertCircle size={20} className="text-yellow-500" /> Drift Events</h3>h3>
                                  <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value)}
                                                className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-slate-200">
                                              <option value="all">All</option>option><option value="high">High</option>option><option value="medium">Medium</option>option><option value="low">Low</option>option>
                                  </select>select>
                        </div>div>
                        ...
                        <div key={drift.driftId} className={`p-3 rounded border transition-colors ${drift.severity==='high'?'bg-red-900/20 border-red-700 hover:bg-red-900/30':drift.severity==='medium'?'bg-yellow-900/20 border-yellow-700 hover:bg-yellow-900/30':'bg-blue-900/20 border-blue-700 hover:bg-blue-900/30'}`}>
                                      <div className="flex items-start justify-between">
                                                      <div>
                                                                        ...
                                                                        <div className="text-xs text-slate-500 mt-1">{new Date(drift.timestamp).toLocaleTimeString()}</div>div>
                                                      </div>div>
                                                      <span className={`px-2 py-1 rounded text-xs font-medium whitespace-nowrap ${drift.severity==='high'?'bg-red-900 text-red-200':drift.severity==='medium'?'bg-yellow-900 text-yellow-200':'bg-blue-900 text-blue-200'}`}>{drift.severity}</span>span>
                                      </div>div>
                        </div>div>
                      ))}
                        </div>div>
          ...
          <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                  {[{l:'Decision Episodes',v:episodes.length},{l:'Drift Events',v:drifts.length},{l:'Agents Tracked',v:metrics.length},{l:'Data Points',v:episodes.length*7+drifts.length*4}].map(s=>(
                      <div key={s.l} className="p-4 bg-slate-800 rounded-lg"><div className="text-sm text-slate-400">{s.l}</div>div><div className="text-2xl font-bold">{s.v}</div>div></div>div>
                    ))}
                </div>div>
                <div className="flex gap-3">
                        <button onClick={()=>doExport('json')} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded transition-colors flex items-center gap-2"><Download size={16}/>Export JSON</button>button>
                        <button onClick={()=>doExport('csv')} className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded transition-colors flex items-center gap-2"><Download size={16}/>Export CSV</button>button>
                </div>div>
                <p className="text-xs text-slate-500 mt-4">Exports all decision episodes, drift events, and agent metrics for offline analysis.</p>p>
          </div>div>
        );
}
          ...
          <div className="min-h-screen bg-slate-950 text-slate-100">
                <header className="border-b border-slate-800 bg-slate-900 sticky top-0 z-50">
                        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between flex-wrap gap-3">
                                  ...
                                  <div className="flex items-center gap-4">
                                              ...
                                              <button onClick={loadData} className="text-xs text-slate-400 hover:text-blue-400 transition-colors">{'\u21BB'} Refresh</button>button>
                                              <div className="text-xs text-slate-500">Updated: {lastUpdate}</div>div>
                                  </div>div>
                        </div>div>
                </header>header>
                ...
                <nav className="border-b border-slate-800 bg-slate-900">
                        ...
                        <button key={view} onClick={()=>setSelectedView(view)}
                                      ...
                                      <span className="text-xs text-slate-600 mr-1">{i+1}</span>span>{view === 'iris' ? 'IRIS' : view.charAt(0).toUpperCase()+view.slice(1)}
                        </button>button>
                      ))}
                        </div>div>
                </nav>nav>
                      ...
                      <div className="space-y-6">
                                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                                                ...
                                                <div className="bg-slate-900 rounded-lg border border-slate-800 p-4 flex flex-col items-center justify-center hover:border-slate-700 transition-colors">
                                                                <div className="text-slate-400 text-sm mb-1">System Health</div>div>
                                  ...
                                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                                                                ...
                                                                <ResponsiveContainer width="100%" height={300}>
                                                                                  <AreaChart data={metricsOverTime}>
                                                                                                      <defs>
                                                                                                                            <linearGradient id="gDL" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#60a5fa" stopOpacity={0.3}/><stop offset="95%" stopColor="#60a5fa" stopOpacity={0}/></linearGradient>linearGradient>
                                                                                                                            <linearGradient id="gDR" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.3}/><stop offset="95%" stopColor="#ef4444" stopOpacity={0}/></linearGradient>linearGradient>
                                                                                                        </defs>defs>
                                                ...
                                                <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                                                                <h3 className="text-lg font-semibold mb-4">Agent Comparison Radar</h3>h3>
                                                                <ResponsiveContainer width="100%" height={300}>
                                                ...
                                                <div className="bg-slate-900 rounded-lg border border-slate-800 p-6">
                                                                <h3 className="text-lg font-semibold mb-4">Agent Performance</h3>h3>
                                                                <ResponsiveContainer width="100%" height={300}>
