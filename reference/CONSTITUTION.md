...
| `CONTRACT_FINGERPRINT` | Generated by `constitution_gate.py` | Single digest of the full schema manifest |
| `COMPAT_RULES.md` | Hand-maintained | MAJOR/MINOR/PATCH break classification |
...
See [COMPAT_RULES.md](COMPAT_RULES.md) for the full classification.

- **MAJOR** changes (field removal, type change, enum narrowing) bump the major version and require migration notes with consumer action and rollback assessment.
- **MINOR** changes (new optional field, new schema, enum widening) bump the minor version. Old consumers continue to work without modification.
- **PATCH** changes (description edits, URL updates) do not alter validation behavior.

Every CHANGELOG entry includes the **contract fingerprint** — the SHA-256 digest of the schema manifest — so consumers can verify which contract surface produced an artifact.
