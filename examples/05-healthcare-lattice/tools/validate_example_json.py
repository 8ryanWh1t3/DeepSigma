#!/usr/bin/env python3
"""Validate Healthcare Lattice example JSON files.

Checks that episodes, drift signals, and patch artifacts have required
top-level keys. Uses only stdlib (json, sys, pathlib).
...
# Required keys per artifact type
EPISODE_REQUIRED = {
    "id_fields": ["episode_id", "decision_episode_id", "id"],
    "required": ["title", "severity", "claims_affected", "seal"],
    "recommended": ["domain_primary", "domains_affected", "dlr", "rs", "ds", "mg"],
}

DRIFT_REQUIRED = {
    "id_fields": ["ds_id", "drift_id", "signal_id", "id"],
...
def main() -> None:
    print("Healthcare Lattice — JSON Validation")
    print("=" * 50)
