...
score:
\tmkdir -p out
\t$(PYTHON) -m core score ./data/sample_episodes.json --json > out/score.json

iris:
\tmkdir -p out
\t$(PYTHON) -m core iris query ./data/sample_episodes.json --type WHY --target ep-demo-003 --json > out/iris_why.json
\t$(PYTHON) -m core iris query ./data/sample_episodes.json --type WHAT_DRIFTED --json > out/iris_drift.json
...
```bash
python -m core iris query ./data/sample_episodes.json --type WHY --target ep-demo-003 --json
python -m core iris query ./data/sample_episodes.json --type WHAT_DRIFTED --json
```
""",
    )
