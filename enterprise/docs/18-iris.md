...
> > > | File | Description |
> > > |------|-------------|
> > > | `core/iris.py` | IRIS engine implementation — query resolution, provenance assembly, confidence scoring |
> > > | `specs/iris_query.schema.json` | JSON Schema contract for IRIS query and response formats |
> > > | `core/cli.py` | CLI integration — `coherence iris query` command |
