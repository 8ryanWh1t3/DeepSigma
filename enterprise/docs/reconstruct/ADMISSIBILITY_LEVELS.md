...
| **L7** | Authority-Bound | L6 + authority ledger entry verified — actor held valid, scoped, un-revoked authority at commit time | Unauthorized actors, scope escalation, expired authority |
...
| L7 | L6 + `--verify-authority true --authority-ledger <ledger_path>` |
...
| Authority Ledger | L7 | `authority_ledger_entry_v1.json` | `authority_ledger_append.py` / `authority_ledger_verify.py` |

---

## Admissibility Pack Contents by Level

| File | L0 | L1 | L2 | L3 | L4 | L5 | L6 | L7 |
|------|----|----|----|----|----|----|-----|-----|
| Sealed run JSON | X | X | X | X | X | X | X | X |
| Manifest JSON | X | X | X | X | X | X | X | X |
| Signature `.sig.json` | | | X | X | X | X | X | X |
| Transparency log entry | | | | X | X | X | X | X |
| Multi-sig envelope | | | | | X | X | X | X |
| Commitment roots | | | | | | X | X | X |
| Authority ledger excerpt | | | | | | | | X |
| Replay instructions | X | X | X | X | X | X | X | X |
