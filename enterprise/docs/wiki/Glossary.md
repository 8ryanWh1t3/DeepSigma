...
- **DomainMode**: Base class for IntelOps, FranOps, ReflectionOps, AuthorityOps, and ParadoxOps. Provides `handle(function_id, event, context) → FunctionResult` dispatch and `replay()` for deterministic verification. See `src/core/modes/base.py`
...
- **Event Contracts**: Routing table mapping 67 functions + 79 events to FEEDS topics, subtypes, handler paths, required payload fields, and emitted events. See `src/core/feeds/contracts/routing_table.json`
