...
| **AuthorityOps** | Authority Operations mode. 12 function handlers (AUTH-F01–F12): action intake, actor/resource resolve, policy load, DLR presence check, assumption validate, half-life check, blast radius threshold, kill-switch check, decision gate, audit emit, delegation chain validate. 6 verdict statuses: ALLOW, BLOCK, ESCALATE, EXPIRED, MISSING_REASONING, KILL_SWITCH_ACTIVE. Implementation: `core/modes/authorityops.py`. |
...
| **Cascade Engine** | Cross-domain event propagation engine with 13 declarative rules. When an event in one domain triggers a rule, the cascade engine invokes the target domain's handler with depth limiting to prevent infinite loops. Implementation: `core/modes/cascade.py`. |
| **Event Contracts** | Declarative routing table mapping 48 functions + 51 events to FEEDS topics, subtypes, handler paths, required payload fields, and emitted events. Validated at publish time. Implementation: `core/feeds/contracts/routing_table.json`. |
...
| **Domino Delegation Verifier** | Read-only ceremony record verification tool (core edition). Loads ceremony JSON, recomputes chain seal, validates connectivity, checks TTL, displays fingerprints. Cannot generate keywords or encrypt. File: `core/edge/EDGE_Domino_Delegation_Encryption_Verifier.html`. |
