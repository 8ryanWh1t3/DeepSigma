...
| **ReflectionOps** | Reflection Operations mode. 19 function handlers (RE-F01–F19): episode begin/seal/archive, gate evaluate/degrade/killswitch, non-coercion audit, severity scoring, coherence check, reflection ingest, IRIS resolve, episode replay + institutional memory (precedent ingest, pattern fingerprint, precedent match, knowledge consolidate, temporal recall, knowledge decay, IRIS precedent resolve). Implementation: `core/modes/reflectionops.py`. |
| **AuthorityOps** | Authority Operations mode. 19 function handlers (AUTH-F01–F19): action intake, actor/resource resolve, policy load, DLR presence check, assumption validate, half-life check, blast radius threshold + simulation + propagation + seal, kill-switch check, decision gate, audit emit, delegation chain validate, authority drift detect + history + cross-domain correlation, assumption sweep. 6 verdict statuses: ALLOW, BLOCK, ESCALATE, EXPIRED, MISSING_REASONING, KILL_SWITCH_ACTIVE. Implementation: `core/modes/authorityops.py`. |
| **ActionOps** | Action Operations mode. 19 function handlers (ACTION-F01–F19): commitment intake/validate, deliverable track, deadline check, compliance evaluate, risk assess, breach detect, escalation, remediation, adjust, complete, report + decision accounting (cost record, time-to-decision, value assess, debt detect, ROI compute, budget enforce, accounting report). Implementation: `core/modes/actionops.py`. |
...
| **Cascade Engine** | Cross-domain event propagation engine with 27 declarative rules. When an event in one domain triggers a rule, the cascade engine invokes the target domain's handler with depth limiting to prevent infinite loops. Implementation: `core/modes/cascade.py`. |
| **Event Contracts** | Declarative routing table mapping 79 functions + 91 events to FEEDS topics, subtypes, handler paths, required payload fields, and emitted events. Validated at publish time. Implementation: `core/feeds/contracts/routing_table.json`. |
| **Institutional Memory** | Precedent registry, pattern fingerprinting, knowledge consolidation, temporal decay. Extension of ReflectionOps (RE-F13–F19). Implementation: `core/institutional_memory/`. |
| **Drift Radar** | Cross-domain drift intelligence surface — correlation, trending, forecasting, remediation prioritization. Sits above domain modes. Implementation: `core/drift_radar/`. |
| **Decision Accounting** | Cost tracking, value scoring, debt engine, ROI computation, budget enforcement. Extension of ActionOps (ACTION-F13–F19). Implementation: `core/decision_accounting/`. |
...
| **Domino Delegation Encryption** | 4-of-7 Shamir threshold encryption ceremony using physical domino tiles as co-presence proof. Seven participants each draw a domino tile, chain them, receive Shamir keywords, and can encrypt/decrypt sensitive text via AES-256-GCM by reconstructing the secret with any 4+ keywords and a passphrase. TTL-gated (1 hour). File: `enterprise/edge/EDGE_Domino_Delegation_Encryption.html`. |
| **Domino Delegation Verifier** | Read-only ceremony record verification tool (core edition). Loads ceremony JSON, recomputes chain seal, validates connectivity, checks TTL, displays fingerprints. Cannot generate keywords or encrypt. File: `edge/EDGE_Domino_Delegation_Encryption_Verifier.html`. |
| **Shamir Secret Sharing (SSS)** | Threshold cryptographic scheme where a secret is split into N shares with threshold K (4-of-7 in Domino ceremony). Any K shares reconstruct the original; fewer than K reveal nothing (information-theoretic security). Implemented over GF(256) with AES irreducible polynomial. |
| **Ceremony JSON** | Public verifiable record of a Domino Delegation Encryption ceremony. Contains chain fingerprint, participant list, keyword fingerprints, session ID, and TTL window. Never includes actual keywords, the reconstructed secret, or the passphrase. |
| **Keyword Fingerprint** | Domain-separated SHA-256 hash of a Shamir keyword share: `SHA-256("KW" + x_byte + share_bytes)`, truncated to first 16 hex characters. Proves keyword-to-ceremony binding without revealing the keyword itself. |
