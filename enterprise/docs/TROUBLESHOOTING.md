...
**Symptom:** Any `python -m core ...` command fails with import error.
...
**Symptom:** Import of `core` raises a syntax or type error immediately.
...
**Verify:**
```bash
python --version && python -c "import core; print('OK')"
```
...
**Symptom:** Test raises `FileNotFoundError` for a path under `examples/` or `core/examples/`.
...
ls examples/episodes/ core/examples/
```
...
If sample data was modified, restore from git: `git checkout core/examples/`.
...
python -m core mg export ./core/examples/sample_episodes.json \
...
### 14. `python -m core.examples.drift_patch_cycle` — `ModuleNotFoundError`
...
```bash
pip install -e .
# Now use: coherence score ./core/examples/sample_episodes.json
# Or always use: python -m core score ...
```
...
**Symptom:** `python -m core iris query --type WHY --target ep-001` returns no data.
...
"
# Then use a real ID:
python -m core iris query --type WHY --target <real-id>
```
