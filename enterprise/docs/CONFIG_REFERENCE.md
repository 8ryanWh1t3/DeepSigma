...
1. **CLI arguments** — passed directly to `python -m core ...` commands
...
**Entry point:** `python -m core <subcommand>` or `coherence <subcommand>` (after `pip install -e .`)

#### `score`

```
python -m core score <episodes_path> [--json]
```
...
```
python -m core audit <episodes_path>
```
...
```
python -m core mg export <episodes_path> --format=<fmt> [--output=<path>]
```
...
```
python -m core iris query --type <TYPE> [--target <id>] [--json]
```
...
```
python -m core demo <episodes_path>
```
