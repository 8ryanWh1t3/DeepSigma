...
> > > > > ## What changed
> > > > >
> > > > > The original DLR (`core/dlr.py`) was episode-centric: it extracted flat metadata from a sealed DecisionEpisode (DTE ref, action contract fields, verification result, policy stamp, outcome code). It had no concept of claims, no rationale graph, and no freshness tracking.
> > > > >
...
> > > > >
> > > > > The existing `core/dlr.py` `DLREntry` dataclass maps to the claim-native DLR as follows:
> > > > >
...
> > > > > | Original DLR (preserved) | `core/dlr.py` |
> > > > >
> > > > > ---
> > > > >
> > > > > ## What comes next
