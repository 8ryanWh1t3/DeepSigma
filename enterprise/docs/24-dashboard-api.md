...
The API server loads real DecisionEpisodes and DriftEvents from the sample corpus, runs the full `core` governance pipeline (DLR / RS / DS / MG), and serves the results as JSON. Includes SSE streaming, IRIS query support, and webhook endpoints for SharePoint and Power Automate.
...
| Drift data | `core/examples/sample_drift.json` |
