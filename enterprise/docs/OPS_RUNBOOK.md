...
| Run Money Demo (canonical) | `python -m core.examples.drift_patch_cycle` |
| Run retention sweep | `deepsigma retention sweep --tenant tenant-alpha` |
| Score episodes | `python -m core score ./core/examples/sample_episodes.json --json` |
| Run full test suite | `pytest tests/ -v` |
| Run with coverage | `pytest --cov=core --cov-report=term-missing` |
...
| Export Memory Graph | `python -m core mg export ./core/examples/sample_episodes.json --format=json` |
| IRIS: Why this decision? | `python -m core iris query --type WHY --target ep-001` |
| Check artifact paths | `ls examples/episodes/ examples/drift/ core/examples/` |
...
# 3. Score coherence
python -m core score ./core/examples/sample_episodes.json
...
| Python import errors | `python -c "import core; print('ok')"` |
...
| Sample episodes (core) | `core/examples/` | `core/examples/sample_episodes.json` |
| Drift events | `examples/drift/` | `examples/drift/freshness_drift.json` |
| Demo drift | `core/examples/` | `core/examples/sample_drift.json` |
...
**Symptoms:**
- `python -m core iris query --type WHY --target <id>` hangs or exceeds 60s
...
1. **Check episode count** — large corpora increase scan time:
   ...
   eps = json.loads(Path('core/examples/sample_episodes.json').read_text())
...
4. **Test the IRIS query in isolation:**
   ```bash
   time python -m core iris query --type WHY --target ep-001
   ```

5. **Escalate** if all of the above pass but latency still exceeds 60s — file a GitHub issue with:
   - Output of `python -m core score <path> --json`
   - Episode count
...
- [ ] `python -m core score ./core/examples/sample_episodes.json` — grade ≥ C
