...
The Reconciler compares DLR entries, drift signals, and Memory Graph nodes to find mismatches between artifacts produced by the core pipeline. It returns structured repair proposals and can auto-apply safe fixes.

**Source:** `core/reconciler.py`

---

## Setup

No extra dependencies. Part of the `core` package.

```python
from core.reconciler import Reconciler, RepairKind, RepairProposal
```
...
```python
from core import DLRBuilder, DriftSignalCollector, MemoryGraph
...
| Source | `core/reconciler.py` |
| DLR | `core/dlr.py` |
| Drift signals | `core/ds.py` |
| Memory Graph | `core/mg.py` |
