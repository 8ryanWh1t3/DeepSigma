...
class TestSQLiteStorageDLR:
    ...
    def test_upsert(self, tmp_path):
        b = SQLiteStorageBackend(tmp_path / "test.db")
        b.save_dlr(SAMPLE_DLR)
        updated = {**SAMPLE_DLR, "outcome_code": "failure"}
        b.save_dlr(updated)
        dlr = b.get_dlr("DLR-abc123")
        assert dlr["outcome_code"] == "failure"

    def test_list(self, tmp_path):
        b = SQLiteStorageBackend(tmp_path / "test.db")
        b.save_dlr(SAMPLE_DLR)
        b.save_dlr({**SAMPLE_DLR, "dlr_id": "DLR-def456", "recorded_at": "2026-02-19T02:00:00Z"})
