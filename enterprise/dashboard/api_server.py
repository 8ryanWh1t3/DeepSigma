...
the full core governance pipeline, exposing the results as JSON for
...
_EP_DIRS = [
    ...
    REPO_ROOT / "core" / "examples",
]
_DRIFT_PATHS = [
    REPO_ROOT / "core" / "examples" / "sample_drift.json",
...
def get_coherence():
    """Run the full core pipeline and return the scored report."""
    episodes = _load_episodes()
    drifts = _load_drifts()
    if not episodes:
