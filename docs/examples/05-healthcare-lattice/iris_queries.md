...
```bash
python -m coherence_ops iris query --type WHY --target ep-hc-002
```
...
```bash
python -m coherence_ops iris query --type WHY --target ep-hc-003
```
...
```bash
python -m coherence_ops iris query --type WHY --target ep-hc-004
```
...
```bash
python -m coherence_ops iris query --type SHOW --target staffing-correlation
```
