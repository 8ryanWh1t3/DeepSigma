...
IRIS resolves operator queries (WHY, WHAT_CHANGED, WHAT_DRIFTED, RECALL, STATUS)
by walking the MemoryGraph's claim topology and DLR rationale graph.
...
class IRISEngine:
    ...
    def _resolve_why(self, query: IRISQuery, query_id: str,
                                          warnings: List[str]) -> IRISResponse:
                                                      ...
                                                      if query.claim_id:
                                                                      ...
                                                                      if "error" in result:
                                                                                          return IRISResponse(
                                                                                                                  query_id=query_id, query_type=QueryType.WHY,
                                                                      ...
                                                                      for ev in result.get("evidence", []):
                                                                          ...
                                                                          data=result, provenance=provenance, warnings=warnings,
                                                                      )
    ...
    def _resolve_what_changed(self, query: IRISQuery, query_id: str,
                                                             warnings: List[str]) -> IRISResponse:
                                                                         ...
                                                                         if query.episode_id:
                                                                                         ...
                                                                                         return IRISResponse(
                                                                                             query_id=query_id, query_type=QueryType.WHAT_CHANGED,
                                                                                             ...
                                                                                             data=patches, warnings=warnings,
                                                                                         )
                                                                                     if query.claim_id:
                                                                                                     result = self._mg.query("claim", claim_id=query.claim_id)
                                                                                                     ...
                                                                                                     return IRISResponse(
                                                                                                         ...
                                                                                                         data={"supersedes": supersedes, "claim": result}, warnings=warnings,
                                                                                                         )
                                                                                                 return IRISResponse(
                                                                                                                 query_id=query_id, query_type=QueryType.WHAT_CHANGED,
                                                                                                                 status=ResolutionStatus.ERROR,
    ...
    def _resolve_what_drifted(self, query: IRISQuery, query_id: str,
        ...
        return IRISResponse(
                        query_id=query_id, query_type=QueryType.WHAT_DRIFTED,
    ...
    def _resolve_recall(self, query: IRISQuery, query_id: str,
        ...
        return IRISResponse(
                        query_id=query_id, query_type=QueryType.RECALL,
                        status=ResolutionStatus.RESOLVED,
    ...
    def _resolve_status(self, query: IRISQuery, query_id: str,
        ...
        return IRISResponse(
                        query_id=query_id, query_type=QueryType.STATUS,
                        status=ResolutionStatus.RESOLVED,
                        summary=f"Health: {health}. {msg} Claims: {claim_count}.",
                        data={"health": health, "stats": stats}, warnings=warnings,
        )
